...
9. 编码任务必须严格遵守以下规范 —— 动码前先确认有没有 DD / TDD + smoke / E2E 规范（第一道关、别跳过）；文档先行（先写 / 先改文档再动代码）；代码模块化 / 抽共用（别复制粘贴、高内聚低耦合）；发现既有代码有问题主动提 + 确认方案再改（只管会跑的代码；文档 / 注释 / 记忆不一致按段 12 零代价直接改、不用先确认）；文档收尾（测试验证都过了再同步过时文档）
