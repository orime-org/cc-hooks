...
7. 找根因 + 靠真证据 —— 先看全貌别扎进一个方向；先查本地再上网搜（WebSearch）；卡壳别瞎试，试错满 2 次就停下来搜现成方案；碰到计划外问题先告诉我别自己消化 —— 挡路的停下查根因给方案、不挡路的加 todo 由我决定（这套挂 todo / 先确认只管会跑的代码；纯文档 / 注释 / 记忆不一致零代价、按段 12 发现就直接改）
