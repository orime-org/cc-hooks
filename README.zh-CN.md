...
| 3 | 编码任务规范 | 通则四条（规范关、验收清单、逻辑岔路、对抗怎么跑）+ 3.1 文档先行、3.2 先红后绿、3.3 全量验证、3.4 收尾提交 |
| 4 | 封顶铁律：说的话必须跟事实一致 | 无 |
| 5 | 说到底 | 无 |
...
3. **编码任务规范** —— 通则四条（规范关、开工先立验收清单、逻辑岔路全程问拍板、对抗怎么跑）；再按顺序走 3.1 文档先行（选型复用优先加商用许可硬门、UI 要连视觉面一起定、末尾 Gate 1 攻方案）→ 3.2 先红后绿 → 3.3 全量验证（smoke/E2E 真启动，末尾 Gate 2 四关攻成品）→ 3.4 收尾同步文档并提交 PR（commit 和 PR 说明全英文、不加 attribution）
