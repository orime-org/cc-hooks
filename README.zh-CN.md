...
10. 对抗验证 —— 独立成顶层段（派独立对抗者、两道关都攻）。10.1 铁律（独立对抗者别自评、按风险 / 交付边界触发、攻 → 修 → 复攻到无新洞、上限普通 3 高危 5、咬到固化成测试、不过关停下报我）；10.2 Gate 1 设计对抗（写代码前、9.1 触发）；10.3 Gate 2 成品对抗三 lens：代码对抗（崩不崩：边界 / 畸形 / 并发 / 注入）/ **代码质量对抗**（工业级：模块化 / 同类问题一致 / DRY）/ 方案根因对抗（是不是创可贴）；10.4 结果 —— 复核走段 12 收口底线 + **每轮累积报小结**（第 N 轮列第 1~N 轮：攻了哪些 lens、咬出啥、修了啥、还剩啥）
