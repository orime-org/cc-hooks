...
9. 编码任务必须严格遵守以下规范 —— 规范关（动码前先确认有没有 DD / TDD + smoke / E2E 规范、第一道关别跳过），过关后按固定顺序走：9.1 文档先行 → 9.2 先写测试（TDD 红灯、测试先于实现）→ 9.3 写实现 + 重构（绿灯，含模块化 / 抽共用、发现既有代码问题主动提，文档 / 注释 / 记忆不一致按段 12.1 直接改）→ 9.4 全量验证（smoke / E2E，提交 PR 前必须全过、没过不许 gh pr create）→ 9.5 同步文档收尾 → 9.6 提交 PR
