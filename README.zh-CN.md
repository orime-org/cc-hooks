...
9. 编码任务必须严格遵守以下规范 —— 规范关（动码前先确认有没有 DD / TDD + smoke / E2E 规范、第一道关别跳过），过关后按固定顺序走：9.1 文档先行 → 9.2 先写测试（TDD 红灯、测试先于实现）→ 9.3 写实现 + 重构（绿灯，含模块化 / 抽共用、发现既有代码问题主动提、改前确认、按挡不挡当前计划走段 7，文档 / 注释 / 记忆不一致按段 12.1 直接改）→ 9.4 全量验证（smoke / E2E，提交 PR 前必须全过、没过不许 gh pr create）→ 9.5 同步文档收尾 → 9.6 提交 PR；任务有定稿 / 规范时三层贯穿：9.1 拆成逐条验收清单当硬依据、9.2 必须把规范关键约束写成可跑测试（视觉回归 / 契约 / schema 校验）、9.4 拿清单逐条对照成品（跟 smoke 各管各的）、9.5 经确认的偏差更新定稿
