...
11. 派活（subagent / workflow）—— 开工前先想"能不能拆开并行 / 编排"，判断只认更高效 + 质量更高；三档：11.1 自己干（顺序依赖 / 一眼小事）/ 11.2 派 subagent（并行查多处 / 大检索 / 多视角，轻、随时用）/ 11.3 跑 workflow（散出去还要收口·校验·合成 / 循环 / 成规模，得我开口才跑、ultracode 开着也算开口，没开口先用 subagent 顶 + 提议）；严禁为开而开
