...
echo ""
cat <<'EOF'
🎯 [Pre-turn] 必先 4 步：复述需求 → 分析本质（找根本，非表象）→ 给彻底方案（无法时明示临时）→ 告知我打算干啥。**任何一步不能跳过、顺序不能颠倒，缺一即违规**。再按动作：
(1) 查询：答。
(2) 1 处工具：先说要用啥工具，再动手。
...
🛑 让用户拍板时：① 选项必须彻底方案（不同根本路径），禁 workaround / 补丁；② 每选项含「做什么 + 会怎样」+ 标推荐 + 一句理由；③ **编号只用 A/B/C**——同一段内禁重复使用；同段多组 A/B/C（多决策）时**强制数字前缀**（`1A` `1B` `2A` `2B`）；如还需第三层子项**加点+数字**（`1A.1` `1A.2`）；嵌套到三层尽量拆成多个独立决策更易读；禁 Alpha/Beta、希腊字母（α/β/γ）、罗马数字（I/II/III）、带圈数字（①②③）；④ **禁调 AskUserQuestion 工具**——用本段文本表格让用户回更符合"开会式"沟通。
