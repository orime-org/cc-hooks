...
🛑 让用户拍板时：① 选项必须彻底方案（不同根本路径），禁 workaround / 补丁；② **通俗易懂**（全程）——所有面向用户的内容（含表格 cell / 推荐理由 / 副作用 / 细节描述）都讲得让不懂技术的人也能听明白：「做什么 + 会怎样」+ 标推荐 + 一句理由；禁工程术语 / 项目代号 / 缩写；禁前段类比后段术语化；③ **编号只用 A/B/C**（子项 `A.1` `A.2`），禁 Alpha/Beta、希腊字母（α/β/γ）、罗马数字（I/II/III）、带圈数字（①②③）。
