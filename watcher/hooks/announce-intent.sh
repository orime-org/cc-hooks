...
5. 让用户拍板时：(1) 选项必须彻底方案（不同根本路径），禁 workaround；(2) 强制 markdown 表格——**选项作行不作列**；列头固定 5 列「选 / 做什么 / 会怎样 / 推荐 / 理由」；每选项独占一行；推荐列仅推荐项填"推荐"其他留空；(3) **编号只用 A/B/C**——同一段内禁重复使用；同段多组 A/B/C（多决策）时**强制数字前缀**（`1A` `1B` `2A` `2B`）；二层封顶（`1A` / `1A.1`）；超过强制拆独立决策；禁 Alpha/Beta、希腊字母（α/β/γ）、罗马数字（I/II/III）、带圈数字（①②③）；(4) **禁调 AskUserQuestion 工具**——用 md 表格进行回复，更便于用户整体思考和决策。
...
7. 执行彻底方案不打折：(1) 本应一起改的事禁拆 scope / phase / 下次 PR / "本次只做 X"；(2) 时间 / 复杂度 / PR 大小不是借口。
