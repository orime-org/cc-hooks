...
```bash
python3 watcher/scripts/smoke-visual-adversary.py
```

`extract-tokens.js` 的断言用临时目录造 fixture；`inject-collect.js` 的断言由 `watcher/scripts/va-browser-check.js` 在真实 chromium 里跑。
