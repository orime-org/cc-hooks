...
- **真根因诊断**：本轮处理是不是真解决根本问题（按 announce 3.2.2「结论只认证据，不认记忆、经验、推测、类比」+ 3.2.6「一次解决完整，不得主动拆阶段」做自检）
- **实证手段**：按 announce 3.2.2 的取证顺序做了哪些实证（本地代码/文档/git log → 官方文档/WebSearch/GitHub issue → 最小复现或实际验证）
