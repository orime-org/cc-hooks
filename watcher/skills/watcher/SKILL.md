...
#### 1.0 先确定本轮要审计「哪些文件夹」（描述式发现，不是机械枚举路径）

一个 CC 会话可能同时在管多个本地文件夹（每个都可能各有自己的 `.watcher/` 三件套）。**别只盯 cwd 那一个**——本轮 audit 的覆盖范围 = 下面两类文件夹的**并集**：

| 类别 | 怎么判断 | 例子 |
|---|---|---|
| 直接涉及 | 本轮真的动过它的文件（Edit / Write 改过，或为改而 Read 过） | 本轮改了 `A/src/api.ts` → A 是直接涉及 |
| 间接涉及（描述式，关键） | 本轮没碰它的文件，但本轮改动在语义上让它的文档会过时——跨项目依赖（上游 API / 共享 schema / 子域 / SDK / 公共组件 / 环境变量） | 本轮改了 A 的对外 API，B 依赖这个 API → 即使没碰 B 任何文件，B 也要审 |
...
1. 列出 agent 记忆文件（全局一份，不分文件夹）
2. `ls <folder-root>/`
3. `ls <folder>/docs/ 2>/dev/null`
4. `find <folder> -maxdepth 2 -name "*.md" -not -path "*/node_modules/*" -not -path "*/.git/*"`
5. 读该文件夹的 README.md / CLAUDE.md / 主要 docs
6. **检查 `<folder>/.watcher/` 是否存在**（只认这个文件夹自己目录下的，不往父目录上爬）

每个文件夹各输出一份文件清单（内部用），每个文件标"评估过 / 要改 / 不用改"。

### 第二步：检测项目身份（对每个发现的文件夹各判一次）

对第一步发现的**每个**文件夹，按它有没有 `.watcher/` + 它是直接还是间接涉及，分级处理：
...
<本轮覆盖的文件夹清单（标直接 / 间接涉及）+ 各自用的是它自己目录下的 .watcher（不上爬；没配的已按分级提醒）等首行声明>（如有直接涉及的文件夹缺 .watcher/，这里逐字输出首行高亮警告）
...
- **真根因诊断**：本轮处理是不是真解决根本问题——结论有没有直接证据支撑（不认记忆、经验、推测、类比），是不是一次解决完整、没有主动拆阶段
- **实证手段**：做了哪些实证，按取证顺序列出——本地代码/文档/git log/配置 → 官方文档/WebSearch/GitHub issue → 最小复现或实际验证
- **workaround 明示**（如有）：本轮如果有任何治标不治本的临时方案，必须在这里明示
...
**段结构规则**：每个段用 `## N. xxx` 二级标题写顶层段；段编号从 1 开始重新数，每次新 audit 都独立编号。
...
**跨项目 / 多文件夹改动**：本轮覆盖的文件夹按第一步「描述式发现」定（直接涉及 ∪ 间接涉及，问"会不会让 X 的文档过时"），每个文件夹各跑一次完整盘点（各自只认它自己目录下的 `.watcher/`、不向上爬）；缺 `.watcher/` 按第二步分级（直接涉及刷首行高亮、间接涉及只通用规则审 + 「未处理」轻提）；摘要的文档变更按文件夹分组。
