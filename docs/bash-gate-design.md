...
产出物按 1.2 术语表判定：脚本和 `hooks.json` 都是**生产代码**；级别按 3.1.2 判定为**开发级**（只在开发过程中使用），按 3.1.3 开发级不进三类。
...
| 主分支保护 | 命令含 git commit | `git -C "$cwd" branch --show-current` 不是 main 或 master | `exit 2`，stderr 报当前分支名并要求先建任务分支 |
