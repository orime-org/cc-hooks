...
9. Strict coding-task rules — spec gate (confirm DD / TDD + smoke / E2E specs exist before coding, gate one, don't skip), then a fixed order: 9.1 docs-first → 9.2 tests-first (TDD red, test before implementation) → 9.3 implement + refactor (green; incl. modularize/extract-shared, surface & confirm-before-fixing pre-existing bad code, route by blocking-or-not per segment 7, doc/note/memory mismatches fixed directly per segment 12.1) → 9.4 full verify (smoke / E2E, must all pass before PR) → 9.5 sync docs → 9.6 open PR. When the task has an authoritative spec/mock, woven through: 9.1 break it into a per-item checklist as the binding source, 9.2 MUST encode its key constraints as runnable tests (visual regression / contract / schema), 9.4 check the build against the checklist item-by-item (separate from smoke), 9.5 update the spec on confirmed deviations
