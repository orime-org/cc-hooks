...
9. Strict coding-task rules — spec gate (confirm DD / TDD + smoke / E2E + logic-clarification specs exist before coding, gate one, don't skip), then a fixed order: 9.1 docs-first (before writing the doc, ask me the logic forks as a table — only real forks, use a sensible default + flag when one exists) → 9.2 tests-first (TDD red, test before implementation; E2E not forced red here, deferred to 9.4) → 9.3 implement + refactor (green; incl. modularize/extract-shared, ask me on new logic forks per segment 7, surface & confirm-before-fixing pre-existing bad code, doc/note/memory mismatches fixed directly per segment 12.1) → 9.4 full verify (smoke / E2E; UI/app smoke MUST really launch the app + drive a real browser over http, no mock-faking; must all pass before PR) → 9.5 adversarial verify (after green, an independent adversary tries to break it, risk-triggered, loop attack→fix→re-attack until a round finds nothing new — cap 3 normal / 5 high-risk, biting cases become permanent tests) → 9.6 sync docs → 9.7 open PR. When the task has an authoritative spec/mock, woven through: 9.1 break it into a per-item checklist as the binding source, 9.2 MUST encode its key constraints as runnable tests (visual regression / contract / schema), 9.4 check the build against the checklist item-by-item (separate from smoke), 9.6 update the spec on confirmed deviations
