...
9. Strict coding-task rules — spec gate (confirm DD / TDD + smoke / E2E + logic-clarification specs exist before coding, gate one, don't skip), then a fixed order: 9.1 docs-first (reuse-first gate: before writing your own code, search for a battle-tested OSS library — esp. frontend — and adopt it rather than reinventing the wheel; any adopted library MUST be free for commercial use — permissive license only (MIT / Apache-2.0 / BSD / ISC), no viral copyleft (GPL / AGPL), no non-commercial (CC-BY-NC), no source-available-restricted (BSL / SSPL / Elastic / Redis RSAL); read the actual LICENSE / package.json license field, ask me when unsure (incl. MPL / LGPL); then before writing the doc, ask me the logic forks as a table — only real forks, use a sensible default + flag when one exists; after the doc, before code, an independent adversary attacks the solution's premise — "doesn't fix the root, or reinvents an existing wheel → redo the design") → 9.2 tests-first (TDD red, test before implementation; E2E not forced red here, deferred to 9.4) → 9.3 implement + refactor (green; incl. modularize/extract-shared, ask me on new logic forks per segment 7, surface & confirm-before-fixing pre-existing bad code, doc/note/memory mismatches fixed directly per segment 12.1) → 9.4 full verify (smoke / E2E; UI/app smoke MUST really launch the app + drive a real browser over http, no mock-faking; must all pass before PR) → 9.5 adversarial verify (after green, an independent adversary tries to break it, risk-triggered, loop attack→fix→re-attack until a round finds nothing new — cap 3 normal / 5 high-risk, biting cases become permanent tests; after the code attack, a second layer — root-cause adversarial — attacks whether it fixes the root vs a band-aid) → 9.6 sync docs → 9.7 open PR (commit + PR description all in English, Conventional Commits format, no attribution trailer). When the task has an authoritative spec/mock, woven through: 9.1 break it into a per-item checklist as the binding source, 9.2 MUST encode its key constraints as runnable tests (visual regression / contract / schema), 9.4 check the build against the checklist item-by-item (separate from smoke), 9.6 update the spec on confirmed deviations
10. PR after-care — watch CI, post the full PR url after creating it, clean up branches after merge
11. Delegation (subagent / workflow) — before starting, ask "can this be split to run in parallel / orchestrated"; judge only by more-efficient + higher-quality. Three tiers: 11.1 do it yourself (sequential / trivial), 11.2 spawn subagents (parallel multi-folder, big searches, multi-angle reviews — light, always available), 11.3 run a workflow (fan-out that needs collect/verify/synthesize, loops, large-scale — only when I've opted in; ultracode being on counts as opt-in; otherwise use subagents + propose); never spawn just to spawn. Closing rule: a delegated verdict (subagent / workflow, incl. 9.5's adversarial ruling) is INPUT, not a verdict — never rubber-stamp "clean / 0 holes / all green"; you own the final call. Confirm it actually ran (process trace, not an empty / cached / crashed false-green) and spot-check the key conclusions, risk-scaled (not a full re-run). Hard anti-regress: do this second check YOURSELF — never spawn another workflow to check the last one. This is distinct from 9.5's "don't test yourself" (9.5 = who attacks, independent; this = whether you trust the result, you own it)
