...
| 3 | Coding-task rules | Four standing rules (spec gate, acceptance checklist, logic forks, adversarial) + 3.1 docs-first, 3.2 red then green, 3.3 full verify, 3.4 wrap up |
...
3. **Coding-task rules** — four rules that hold throughout (spec gate, acceptance checklist up front, logic forks always come to me, how the adversarial runs); then a fixed order: 3.1 docs-first (reuse-first plus a commercial-license gate, UI designs must cover the visual side, Gate 1 attacks the plan at the end) → 3.2 red then green → 3.3 full verify (smoke/E2E on a really launched app, Gate 2's four lenses attack the build at the end) → 3.4 wrap up, sync docs and open the PR (commit and PR text in English, no attribution trailer)
