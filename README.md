...
2. Segment structure — Markdown headings, numbering whitelist, no fake tables; closes with a pre-send self-scan (banned symbols → legal numbering / missing tables → add them / segment numbering + 4-step intent / no AskUserQuestion → decision table) to catch know-the-rule-but-slip violations
...
9. Strict coding-task rules — spec gate (confirm docs / TDD + smoke / E2E + fork-clarification specs exist before coding, gate one, don't skip), then a fixed order: 9.1 docs-first (reuse-first gate: before writing your own code, search for a battle-tested OSS library — esp. frontend — and adopt it rather than reinventing the wheel; any adopted library MUST be free for commercial use — permissive license only (MIT / Apache-2.0 / BSD / ISC), no viral copyleft (GPL / AGPL), no non-commercial (CC-BY-NC), no source-available-restricted (BSL / SSPL / Elastic / Redis RSAL); read the actual LICENSE / package.json license field, ask me when unsure (incl. MPL / LGPL); then before writing the doc, ask me the logic forks as a table — only real forks, use a sensible default + flag when one exists; after the doc, before code, run the Gate 1 design adversarial — see 9.5; for UI work the design must also cover the visual side — layout / states / motion / consistency — and new-or-heavily-reworked visuals get a viewable demo (standalone HTML — http or an artifact, each project decides) for sign-off before implementation, while changes to already-runnable UI are dogfooded in the real app instead) → 9.2 tests-first (TDD red, test before implementation; E2E not forced red here, deferred to 9.4) → 9.3 implement + refactor (green; incl. modularize/extract-shared, ask me on new logic forks per segment 7, surface & confirm-before-fixing pre-existing bad code, doc/note/memory mismatches fixed directly per segment 12.1) → 9.4 full verify (smoke / E2E; UI/app smoke MUST really launch the app + drive a real browser over http via MCP tools, not the bash sandbox, no mock-faking; must all pass before PR) → 9.5 adversarial verify — split into 10 single-purpose rules: **who attacks** (an independent adversary, never self; it **finds but never judges — everything comes back raw**, and no verdict/filter layer may be nested inside the run); **when to attack / when it's waived** (Gate 1 in the design phase before code, then a docs-only PR; Gate 2 when a code PR is due and only after smoke/dogfood really passes; pure styling / copy / config-value / comment changes with no attack surface waive both gates); **how many rounds** (attack → fix → re-attack until no new holes, cap 3 for ordinary changes and 5 for high-risk ones, every confirmed behavioural hole gets frozen into a permanent test); **how holes are classified** (**behavioural** = crashes / wrong logic / root cause untreated / docs-or-names contradicting code / duplicated logic / missing error paths / giant functions / same thing written differently in many places, criterion = enumerable and freezable into a check; **"new hole"** = a behavioural hole none of the previous rounds reported, excluding ones ruled false and all descriptive ones; **descriptive** = wording / comments / naming / style where the semantics are fine and it's purely taste — fixed once, never re-enters the loop, never counts as new, still goes on the per-round table); **what Gate 1 attacks / what Gate 2 attacks** (three lenses: does it crash / code quality read-only / root-cause-vs-band-aid); **how to adjudicate** (per item on evidence, never sampled; first confirm the attack actually ran — no trace means not verified, send it back; reproduce what can be run, re-read file:line for what can't; evidence = a real hole, no repro but a stateable trigger = listed for me to decide, can't state one = false); **how to fix** (**group before fixing** — cluster the real ones to fix by root cause or pattern, fix each group in one pass, factor out shared code, never patch item by item; **fixing a hole also follows segment 7 root-cause and segment 8 thorough-solution** — don't just edit the sentence the adversary quoted, ask why this class of hole appeared and whether siblings exist, because a point patch guarantees new holes next round; anything reported as "fixed" is verified per item too); **how to report** (every round, only once attacks and fixes and every item are done, two tables — a per-hole table for this round with columns group / lens / type / verdict (fixed · reported-unfixed · pending-nonblocking · false) / evidence, plus a cumulative rounds 1–N table, each preceded by a line counting the verdicts, with pending-nonblocking items explained in their own paragraph); **what counts as passing** (**convergence = both conditions** — "new holes" at 0 for N consecutive rounds AND everything ruled to-fix actually fixed; hitting the round cap with holes left, judging a fix a band-aid, anything left unfixed, or a descriptive hole resurfacing all mean stop and report to me, and a blocker gets reported the moment it surfaces rather than saved for the summary) → 9.6 sync docs → 9.7 open PR (commit + PR description all in English, Conventional Commits format, no attribution trailer). When the task has an authoritative spec/mock, woven through: 9.1 break it into a per-item checklist as the binding source, 9.2 MUST encode its key constraints as runnable tests (visual regression / contract / schema), 9.4 check the build against the checklist item-by-item (separate from smoke), 9.6 update the spec on confirmed deviations
10. PR after-care (any PR) — watch CI, post the full PR url after creating it, clean up branches after merge
11. Delegation (subagent / workflow) — before starting, ask "can this be split to run in parallel / orchestrated"; judge only by more-efficient + higher-quality. Three tiers: 11.1 do it yourself (sequential / trivial), 11.2 spawn subagents (parallel multi-folder, big searches, multi-angle reviews — light, always available), 11.3 run a workflow (fan-out that needs collect/verify/synthesize, loops, large-scale — decide on your own when it's warranted, no opt-in needed); never spawn just to spawn. Closing rule: a delegated verdict (subagent / workflow — general, all delegation) is INPUT, not a verdict — never rubber-stamp "clean / 0 holes / all green"; you own the final call. Confirm it actually ran (process trace, not an empty / cached / crashed false-green), then **split by type** — adversarial (the holes and "fixed" claims coming back from Gate 1 / Gate 2) is adjudicated item by item on evidence, never sampled (see 9.5); every other delegation (search / research / retrieval) gets the key conclusions spot-checked, risk-scaled (not a full re-run). Hard anti-regress: do this second check YOURSELF — never spawn another workflow to check the last one. (Adversarial verdicts re-check through this rule too — boundary in 9.5.)
