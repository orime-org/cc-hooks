...
9. Strict coding-task rules — first confirm DD / TDD + smoke / E2E specs exist (gate one, don't skip); docs-first (write/fix docs before any code); modularize & extract shared code (no copy-paste, high cohesion / low coupling); surface & confirm-before-fixing pre-existing bad code (code only; doc/note/memory mismatches → fix directly per segment 12); docs-after — once all tests/checks pass, sync any stale docs
