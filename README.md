...
9. Strict coding-task rules — spec gate (confirm DD / TDD + smoke / E2E + logic-clarification specs exist before coding, gate one, don't skip), then a fixed order: 9.1 docs-first (reuse-first gate: before writing your own code, search for a battle-tested OSS library — esp. frontend — and adopt it rather than reinventing the wheel; any adopted library MUST be free for commercial use — permissive license only (MIT / Apache-2.0 / BSD / ISC), no viral copyleft (GPL / AGPL), no non-commercial (CC-BY-NC), no source-available-restricted (BSL / SSPL / Elastic / Redis RSAL); read the actual LICENSE / package.json license field, ask me when unsure (incl. MPL / LGPL); then before writing the doc, ask me the logic forks as a table — only real forks, use a sensible default + flag when one exists; after the doc, before code, run the Gate 1 design adversarial — see 9.5; for UI work the design must also cover the visual side — layout / states / motion / consistency — and new-or-heavily-reworked visuals get a viewable demo (standalone HTML — http or an artifact, each project decides) for sign-off before implementation, while changes to already-runnable UI are dogfooded in the real app instead) → 9.2 tests-first (TDD red, test before implementation; E2E not forced red here, deferred to 9.4) → 9.3 implement + refactor (green; incl. modularize/extract-shared, ask me on new logic forks per segment 7, surface & confirm-before-fixing pre-existing bad code, doc/note/memory mismatches fixed directly per segment 12.1) → 9.4 full verify (smoke / E2E; UI/app smoke MUST really launch the app + drive a real browser over http via MCP tools, not the bash sandbox, no mock-faking; must all pass before PR) → 9.5 adversarial verify — folded in here: an independent adversary (not self) **finds but never judges — everything it found comes back raw, and no verdict/filter layer may be nested inside the adversarial run (no verify/judge stage in the workflow pre-filtering the holes for me); back here, every item is adjudicated on evidence, never sampled** (runnable → actually reproduce it, non-runnable → re-read the file:line and check; evidence = a real hole, no repro but a stateable trigger mechanism = listed for me to decide, can't state one = false; "fixed" claims are verified one by one too) — attacks at two gates — Gate 1 design before code (triggered by 9.1) → a design-doc PR, Gate 2 build when a code PR is being submitted and only after smoke / dogfood passes (trivial no-attack-surface changes like a CSS tweak / copy / config are exempt); attack→fix→re-attack loop-until-dry (cap 3 normal / 5 high-risk), biting cases → permanent tests; only **behavioural** holes count as "new holes" (crash / logic error / unfixed root cause / doc-or-name contradicting the code / duplicated logic / missing error boundary / giant function — the test being "can it be enumerated and frozen into a check"), while **descriptive** ones (wording / comments / a nicer name / style — semantics fine, taste only) are fixed once, never re-enter the loop and never count as new, so the loop can actually converge; unclear → treat as behavioural; a descriptive hole reappearing the round after it was fixed → stop and report; structural holes introduced by this change must be fixed to converge, pre-existing ones are reported without blocking (per 9.3, I decide); Gate 2's 3 lenses: crash (edge / malformed / concurrency / injection) / **code-quality** (modularity / consistency / DRY / craft) / root-cause (band-aid?); verdict re-checked per segment 11's closing rule + two reports every round once attacks and fixes are done (this round's per-hole table + a cumulative rounds 1–N table whose "newly surfaced behavioral holes" column decides convergence); stop & report if not clean → 9.6 sync docs → 9.7 open PR (commit + PR description all in English, Conventional Commits format, no attribution trailer). When the task has an authoritative spec/mock, woven through: 9.1 break it into a per-item checklist as the binding source, 9.2 MUST encode its key constraints as runnable tests (visual regression / contract / schema), 9.4 check the build against the checklist item-by-item (separate from smoke), 9.6 update the spec on confirmed deviations
