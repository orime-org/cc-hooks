...
11. Delegation (subagent / workflow) — before starting, ask "can this be split to run in parallel / orchestrated"; judge only by more-efficient + higher-quality. Three tiers: 11.1 do it yourself (sequential / trivial), 11.2 spawn subagents (parallel multi-folder, big searches, multi-angle reviews — light, always available), 11.3 run a workflow (fan-out that needs collect/verify/synthesize, loops, large-scale — only when I've opted in; ultracode being on counts as opt-in; otherwise use subagents + propose); never spawn just to spawn
