...
| 2 | How to work | 2.1 classify before acting, 2.2 root-cause-first, 2.3 thorough-only, 2.4 do it yourself or delegate, 2.5 a PR is not done when opened, 2.6 surface unplanned problems |
| 3 | Coding-task rules | 3.1 three foundations up front, 3.2 the four problem classes, 3.3 rules shared by all three gates, 3.4 docs-first (Gate 1 at the end), 3.5 red then green, 3.6 full verify (Gate 2 at the end), 3.7 wrap up (Gate 3 and submit) |
...
2. **How to work** — classify before acting (two questions: class one is what we promised and must be polished until smooth, class two is a normal person hitting a state we never promised and only needs a clear message plus a clear next step, class three is illegitimate use and needs one hard gate at the entrance plus an audit trail; treating class two or three as class one is what makes work go in circles); root-cause-first and evidence-backed (survey the whole picture, local before remote, stop and search after two failed attempts, quote a rule's text and source before leaning on it); thorough-only with zero discount; do it yourself or delegate (never spawn just to spawn; what a workflow hands back is input, not a verdict, and the closing check is never delegated again); a PR is not done when it is opened; surface unplanned problems instead of absorbing them
3. **Coding-task rules** — first lay down three foundations (spec gate, acceptance checklist, how forks get raised); 3.2 sorts every problem into four classes, where classes one to three cover user needs and experience while the fourth is code quality as an engineering-quality concern; 3.3 holds the rules shared by all three gates; then a fixed order: 3.4 docs-first (reuse-first plus a commercial-license gate, UI designs must cover the visual side, Gate 1 attacks the plan at the end) → 3.5 red then green → 3.6 full verify (smoke/E2E on a really launched app, Gate 2's four lenses attack the build at the end) → 3.7 wrap up (sync the prose, run Gate 3 against comments and docs, then submit; commit and PR text in English, no attribution trailer)
